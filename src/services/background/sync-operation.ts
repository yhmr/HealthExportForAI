...
import { loadBackgroundSyncConfig, loadLastBackgroundSync, saveLastBackgroundSync } from '../config/backgroundSyncConfig';
...
export async function executeSyncLogic(): Promise<SyncExecutionResult> {
    ...
    try {
            ...
            if (initialized) {
                ...
                if (hasData) {
                    ...
                    if (exportSuccess) {
                        ...
                        await addDebugLog('[SyncOperation] Export request returned false (queued or failed)', 'info');
                    }
                } else {
        ...
        if (queueResult.successCount > 0) {
            await addDebugLog(`[SyncOperation] Queue processed: ${queueResult.successCount} items`, 'success');
