...
export async function exportSpreadsheetAsPDF(
    ...
    try {
        ...
        if (existingFile) {
            ...
            const success = await storageAdapter.updateFile(existingFile.id, pdfBase64, 'application/pdf', true);
        ...
        return { success: false, error: error instanceof Error ? error.message : 'PDFエクスポートエラー' };
    }
}
